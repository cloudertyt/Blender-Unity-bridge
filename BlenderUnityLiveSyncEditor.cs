...
public class BubSnapshotObject
{
    ...
    public float[] scale;
    public bool    transform_only;   // ★ true = only transform changed, skip mesh/material
...
public static class BlenderUnityLiveSyncEditor
{
    ...
    // Perf: node-graph hash per object → skip material rebuild when graph unchanged
    private static readonly Dictionary<string, int> s_nodeGraphHashCache = new();
    ...
    private static void ApplySnapshotEvent(BubSnapshotEvent syncEvent)
    {
        ...
        foreach (var obj in objects)
        {
            ...
            // ★ transform_only: skip expensive mesh/material rebuild
            if (!obj.transform_only)
            {
    ...
    /// <summary>Fast structural hash of a node graph for change detection.</summary>
    private static int ComputeNodeGraphHash(BubNodeGraphData graph)
    {
        ...
        unchecked
        {
            ...
            if (graph.nodes != null)
                ...
                {
                    h = h * 31 + (n.name  ?? "").GetHashCode();
                    h = h * 31 + (n.type  ?? "").GetHashCode();
                    h = h * 31 + (n.op    ?? "").GetHashCode();
                    h = h * 31 + (n.blend ?? "").GetHashCode();
                    h = h * 31 + (n.img_name ?? "").GetHashCode();
                    if (n.inputs != null)
                        foreach (var inp in n.inputs)
                        {
                            h = h * 31 + (inp.name ?? "").GetHashCode();
                            h = h * 31 + inp.dv.GetHashCode();
                        }
                }
            if (graph.links != null)
                foreach (var l in graph.links)
                {
                    h = h * 31 + (l.fn ?? "").GetHashCode();
                    h = h * 31 + (l.fs ?? "").GetHashCode();
                    h = h * 31 + (l.tn ?? "").GetHashCode();
                    h = h * 31 + (l.ts ?? "").GetHashCode();
                }
            return h;
        }
    }
                                  ...
                                  ref bool anyMaterialDirty)
    {
        ...
        if (obj.node_graph != null && obj.node_graph.nodes != null && obj.node_graph.nodes.Length > 0)
        {
            ...
            // ★ Skip material rebuild when node graph hasn't changed
            int graphHash = ComputeNodeGraphHash(obj.node_graph);
            if (!s_nodeGraphHashCache.TryGetValue(meshName, out int lastHash) || lastHash != graphHash)
            {
                ApplyNodeGraphMaterial(renderer, obj.node_graph, matName);
                s_nodeGraphHashCache[meshName] = graphHash;
